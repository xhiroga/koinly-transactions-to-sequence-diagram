...
function processRow(row) {
    ...
    // To Walletが空欄の場合：出金（Withdraw）とみなし、To側は外部アドレス
    else if (fromWallet && fromWallet.trim() !== "" && (!toWallet || toWallet.trim() === "")) {
        return `${fromCurrency} (${fromExchange})->>${fromCurrency} (外部アドレス): ${formatAmount(sumFrom)}${fromCurrency}をWithdraw`;
    }
    // From Walletが空欄の場合：入金（Deposit）とみなし、From側は外部アドレス
    else if ((!fromWallet || fromWallet.trim() === "") && toWallet && toWallet.trim() !== "") {
        return `${toCurrency} (外部アドレス)->>${toCurrency} (${toExchange}): ${formatAmount(sumTo)}${toCurrency}をDeposit`;
    }
    // 両方空欄の場合（想定外）
    else {
        return `${toCurrency} (外部アドレス)->>${toCurrency} (外部アドレス): ${formatAmount(sumTo)}${toCurrency}をDeposit`;
    }
}
...
function extractParticipants(row) {
    ...
    if (fromCurrency) {
        ...
        // 出金操作の場合は「外部アドレス」も参加者として追加
        if (!toWallet || toWallet.trim() === "") {
            participants.push(`${fromCurrency} (外部アドレス)`);
        }
    }
    ...
    if (toCurrency) {
        ...
        // 入金操作の場合は「外部アドレス」も参加者として追加
        if (!fromWallet || fromWallet.trim() === "") {
            participants.push(`${toCurrency} (外部アドレス)`);
        }
    }
...
function calculateYearlyBalanceChanges(transactions) {
    ...
    transactions.forEach(row => {
        ...
        // From側の変動（マイナス）
        if (fromCurrency && fromExchange !== "外部アドレス") {
            // 通貨・取引所ごとの変動量
        ...
        // To側の変動（プラス）
        if (toCurrency && toExchange !== "外部アドレス") {
            // 通貨・取引所ごとの変動量
